...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Jointly refine planner and diffusion for Sonata-3.")
...
def main() -> None:
    ...
    outputs = run_diffusion_training(config=config, logger=logger, joint_refine=True)
    logger.info("Joint refinement checkpoint saved to %s", outputs["best_checkpoint"])
