...
class PianoWithShadowHands(base.PianoTask):
    def __init__(
        ...
        **kwargs,
        ...
        self._disable_key_proximity_reward = disable_key_proximity_reward
        self._disable_smooth_motion_reward = disable_smooth_motion_reward
        self._disable_anticipation_reward = disable_anticipation_reward
    ...
    def _set_rewards(self) -> None:
        ...
        # Additional Rewards
        if not self._disable_key_proximity_reward:
            self._reward_fn.add("key_proximity_reward", self._compute_key_proximity_reward)

        if not self._disable_smooth_motion_reward:
            self._reward_fn.add("smooth_motion_reward", self._compute_smooth_motion_reward)

        if not self._disable_anticipation_reward:
            self._reward_fn.add("anticipation_reward", self._compute_anticipation_reward)
    ...
    # Helper methods.
    def _compute_key_proximity_reward(self, physics: mjcf.Physics) -> float:
        keys_to_press = np.flatnonzero(self._goal_current[:-1])

        if len(keys_to_press) == 0:
            return 0.0

        fingertip_pos = [physics.bind(f).xpos.copy() for f in self.left_hand.fingertip_sites]
        fingertip_pos += [physics.bind(f).xpos.copy() for f in self.right_hand.fingertip_sites]
        ...
        for key in keys_to_press:
            ...
            dists = [np.linalg.norm(key_pos - finger) for finger in fingertip_pos]
            rewards.append(np.exp(-min(dists)))

        return float(np.mean(rewards))

    def _compute_smooth_motion_reward(self, physics: mjcf.Physics) -> float:
        """Reward smoother hand motion using joint velocities."""

        vel_penalty = 0.0

        # Right hand joint velocities
        rh_joints = self.right_hand.joints
        rh_vel = np.array([physics.bind(j).qvel for j in rh_joints])

        # Left hand joint velocities
        lh_joints = self.left_hand.joints
        lh_vel = np.array([physics.bind(j).qvel for j in lh_joints])

        vel_penalty += np.sum(np.square(rh_vel))
        vel_penalty += np.sum(np.square(lh_vel))

        return float(np.exp(-0.05 * vel_penalty))

    def _compute_anticipation_reward(self, physics: mjcf.Physics) -> float:
        """Reward preparing fingers for upcoming notes."""

        future_window = 3
        goal_len = self._goal_state.shape[0]

        future_idx = min(self._t_idx + future_window, goal_len - 1)

        future_goal = self._goal_state[future_idx]
        keys_future = np.flatnonzero(future_goal[:-1])

        if len(keys_future) == 0:
            return 0.0
        ...
        for key in keys_future:
            key_geom = self.piano.keys[key].geom[0]
            key_pos = physics.bind(key_geom).xpos.copy()

            dists = [np.linalg.norm(key_pos - finger) for finger in fingertip_pos]
            rewards.append(np.exp(-0.5 * min(dists)))
